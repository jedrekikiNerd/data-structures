...
{
    int value = user_input_action("Podaj liczbę jaką chcesz dodać: ");
    int priority = user_input_action("Podaj priorytet: ");
    Timer timer;
    timer.start();
...
{
    unsigned int position_to_change = user_input_action_unsigned("Podaj indeks na którym chcesz coś zmienić: ");
    int value = user_input_action("Podaj liczbę jaką chcesz nadpisać podane pole: ");
    int priority = user_input_action("Podaj nowy priorytet: ");
    ...
    if (print_time)
        display_action("Zamiana danych zajęła " + std::to_string(timer.elapsedSeconds()) + "ns");
